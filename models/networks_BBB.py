...
from models.layers.BBB.BBBConv import BBBConv2d as BBBConv2d_NoLRT
from models.layers.BBB_LRT.BBBConv import BBBConv2d as BBBConv2d_LRT
from util.util import gkern_2d

BBBConv2d = BBBConv2d_NoLRT

def weights_init(m):
    classname = m.__class__.__name__
    if classname.find('Conv') != -1 and hasattr(m, 'weight'):  # this is only the case for regular conv not BBBConv
...
class ResnetGenEncoder(ModuleWrapper):
    def __init__(self, input_nc, n_blocks=4, ngf=64, norm_layer=nn.BatchNorm2d,
        ...
        model = [nn.ReflectionPad2d(3),
                 BBBConv2d(input_nc, ngf, kernel_size=7, padding=0,
                           bias=use_bias),
        ...
        for i in range(n_downsampling):
            mult = 2**i
            model += [
                BBBConv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
    ...
    #     print('ResnetGenEncoder.forward', out.size())
    #     return out
...
class ResnetGenShared(ModuleWrapper):
...
class ResnetGenDecoder(ModuleWrapper):
    ...
    def forward(self, input):
        if self.gpu_ids and isinstance(input.data, torch.cuda.FloatTensor):
            return nn.parallel.data_parallel(self.model, input, self.gpu_ids), 0
        return self.model(input), 0


# Define a resnet block
class ResnetBlock(ModuleWrapper):
    def __init__(self, dim, norm_layer, use_dropout, use_bias, padding_type='reflect', n_domains=0):
        ...
        conv_block += [
            BBBConv2d(dim + n_domains, dim, kernel_size=3, padding=p, bias=use_bias),
...
class G_Plexer(Plexer):
    ...
    def encode(self, input, domain):
        output, kl = self.encoders[domain].forward(input)
        if self.sharing:
