...
from . import networks_BBB as networks


class BBBComboGANModel(BaseModel):
    def name(self):
        return 'ComboGANModel'

    def __init__(self, opt, num_train_data):
        super(BBBComboGANModel, self).__init__(opt)

        networks.BBBConv2d = networks.BBBConv2d_LRT if opt.local_reparam else networks.BBBConv2d_NoLRT

        self.num_train_data = num_train_data
        self.kl_beta = opt.kl_beta
    ...
    def backward_G(self):
        encoded_A, kl_encoded_A = self.netG.encode(self.real_A, self.DA)
        encoded_B, kl_encoded_B = self.netG.encode(self.real_B, self.DB)

        # Optional identity "autoencode" loss
        if self.lambda_idt > 0:
            # Same encoder and decoder should recreate image
            idt_A, kl_idt_A = self.netG.decode(encoded_A, self.DA)
            loss_idt_A = self.criterionIdt(idt_A, self.real_A)
            idt_B, kl_idt_B = self.netG.decode(encoded_B, self.DB)
        ...
        # GAN loss
        # D_A(G_A(A))
        self.fake_B, kl_fake_B = self.netG.decode(encoded_A, self.DB)
        pred_fake = self.netD.forward(self.fake_B, self.DB)
        self.loss_G[self.DA] = self.criterionGAN(self.pred_real_B, pred_fake, False)
        # D_B(G_B(B))
        self.fake_A, kl_fake_A = self.netG.decode(encoded_B, self.DA)
        pred_fake = self.netD.forward(self.fake_A, self.DA)
        self.loss_G[self.DB] = self.criterionGAN(self.pred_real_A, pred_fake, False)
        # Forward cycle loss
        rec_encoded_A, kl_rec_encoded_A = self.netG.encode(self.fake_B, self.DB)
        self.rec_A, kl = self.netG.decode(rec_encoded_A, self.DA)
        self.loss_cycle[self.DA] = self.criterionCycle(self.rec_A, self.real_A)
        # Backward cycle loss
        rec_encoded_B, kl_rec_encoded_B = self.netG.encode(self.fake_A, self.DA)
        self.rec_B, kl = self.netG.decode(rec_encoded_B, self.DB)
